...
from lib.test.tracker.mambatrack_mtms_motion import MambaTrackMotion
import lib.test.parameter.mambatrack_motion as rgbt_params_motion
...
def run_sequence(seq_name, seq_home, dataset_name, yaml_name, num_gpu=1, epoch=300, debug=0, script_name='mambatrack'):
    if 'VTUAV' in dataset_name:
        seq_txt = seq_name.split('/')[1]
    else:
        seq_txt = seq_name
    save_name = '{}_testep{}'.format(yaml_name, epoch)
    ...
    if script_name == 'mambatrack':
        ...
        mambatrack = MambaTrack(params, dataset_name)  # "GTOT" # dataset_name
        tracker = MambaVT_RGBT(tracker=mambatrack)
    elif script_name == 'mambatrack_motion':
        params = rgbt_params_motion.parameters(yaml_name, epoch, debug=debug)
        mambatrack = MambaTrackMotion(params, dataset_name)  # "GTOT" # dataset_name
        tracker = MambaVT_RGBT(tracker=mambatrack)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--script_name', type=str, default='mambatrack_motion', help='Name of tracking method().')
    parser.add_argument('--yaml_name', type=str, default='vm256_16x2_ep20_7t1s_motion_pre7_vocab', help='Name of tracking method.')
    ...
    parser.add_argument('--epoch', default=4, type=int, help='epochs of ckpt')
