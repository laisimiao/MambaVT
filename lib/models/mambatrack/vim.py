...
from lib.models.mambatrack.rope import VisionRotaryEmbeddingFast
import random
...
class VisionMamba(nn.Module):
    ...
    def resize_pos_embed(self, new_size, pos_embed_checkpoint):
        ...
        if model_pos_embed_len != pos_embed_checkpoint.shape[-2]:
            embedding_size = pos_embed_checkpoint.shape[-1]

            orig_size = int((pos_embed_checkpoint.shape[-2]) ** 0.5)
            # new_size = int((model_pos_embed_len - 1) ** 0.5)

            patch_pos_embed = pos_embed_checkpoint.reshape(-1, orig_size, orig_size, embedding_size).permute(0, 3, 1, 2)
    ...
    def forward_features(self, z, x, z_frame_ids, concat_mode='tsts', scan_mode='spatial_first', inference_params=None):
        ...
        if self.if_abs_pos_embed:
            ...
            # t_rgb = self.pos_drop(t_rgb)
            # s_rgb = self.pos_drop(s_rgb)
        ...
        if concat_mode == "tsts":
            ...
            t_X = t_X.flatten(1, 2)
            # import ipdb; ipdb.set_trace()
            concat_tokens = torch.cat([t_rgb, s_rgb, t_X, s_X], dim=1)
        elif concat_mode == "ttss":
            t_rgb = t_rgb.flatten(1, 2)
            t_X = t_X.flatten(1, 2)
            concat_tokens = torch.cat([t_rgb, t_X, s_rgb, s_X], dim=1)
        elif concat_mode == "crosst":
            crosst_list = []
            for i in range(t_rgb.shape[1]):
                crosst_list.append(t_rgb[:,i])
                crosst_list.append(t_X[:,i])
            concat_tokens = torch.cat(crosst_list.extend([s_rgb, s_X]), dim=1)
        else:
            raise NotImplementedError('no implement such concat mode!')

        concat_tokens = self.pos_drop(concat_tokens)

        # mamba impl
        residual = None
        hidden_states = concat_tokens
...
def vim_tiny_patch16_224_bimambav2_final_pool_mean_abs_pos_embed_with_midclstok_div2(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        print('Load pretrained Vim missing_keys: ', missing_keys)
        print('Load pretrained Vim unexpected_keys: ', unexpected_keys)
    return model
...
def vim_middle_patch16_224_bimambav2_final_pool_mean_abs_pos_embed_with_midclstok_div2(pretrained=False, **kwargs):
    model = VisionMamba(
        patch_size=16, embed_dim=576, depth=32, rms_norm=True, residual_in_fp32=True, fused_add_norm=True,
        if_abs_pos_embed=True, if_rope=False, if_rope_residual=False, bimamba_type="v2",
        if_cls_token=True, if_devide_out=True, use_middle_cls_token=True, **kwargs)
    if pretrained:
        checkpoint = torch.load(pretrained, map_location="cpu")
...
def vim_base_patch16_224_bimambav2_final_pool_mean_abs_pos_embed_with_midclstok_div2(pretrained=False, **kwargs):
    model = VisionMamba(
...
vim_middle_patch16_224 = vim_middle_patch16_224_bimambav2_final_pool_mean_abs_pos_embed_with_midclstok_div2
vim_base_patch16_224 = vim_base_patch16_224_bimambav2_final_pool_mean_abs_pos_embed_with_midclstok_div2


if __name__ == '__main__':
    h=w=1024

    # net = vim_tiny_patch16_224_bimambav2_final_pool_mean_abs_pos_embed_with_midclstok_div2(img_size=h)
    net = vim_small_patch16_224_bimambav2_final_pool_mean_abs_pos_embed_with_midclstok_div2(img_size=h)  # Number of trainable parameters: 26.908416M. torch.Size([1, 4096, 384])
    # net = vim_base_patch16_224_bimambav2_final_pool_mean_abs_pos_embed_with_midclstok_div2(img_size=h)
    # net = vim_middle_patch16_224_bimambav2_final_pool_mean_abs_pos_embed_with_midclstok_div2(img_size=h)
