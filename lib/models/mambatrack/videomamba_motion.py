...
class VisionMamba(nn.Module):
    def __init__(
            ...
            coor_range=2,
            pre_num=0,
            add_motion_pred=False,
            prompt_embed_type='vocab'
    ):
        ...
        if prompt_embed_type == 'vocab':
            self.motion_embedding = MotionEmbeddings(self.bins*self.coor_range, embed_dim)
            self.motion_bias = torch.nn.Parameter(torch.zeros(embed_dim))
        elif prompt_embed_type == 'sam':
            self.register_buffer(
            "positional_encoding_gaussian_matrix",
            1.0 * torch.randn((2, embed_dim//2)),
        )
        else:
            raise ValueError

        self.add_motion_pred = add_motion_pred
        if add_motion_pred:
            self.motion_pred_embedding = nn.Embedding(4, embed_dim)  # for pred coor accordding to motion cues
    ...
    def forward_features(self, z, x, search_pre_coor, concat_mode, inference_params=None):
        # search_pre_coor: [batch,pre_motion_num,4]   x,y,w,h format
        ...
        # motion
        # search_pre_coor: [batch,pre_motion_num,4]   x,y,x,y format  normalized
        # return: [batch, 4*pre_motion_num]

        
        if self.prompt_embed_type == 'vocab':
            pre_seq_search = search_pre_coor.flatten(1).clamp(-1 * self.magic_num, 1 + self.magic_num)  # [B, 4 * self.pre_num,]
            pre_seq_search = (pre_seq_search + self.magic_num) * (self.bins - 1)
            pre_seq_search = pre_seq_search.to(torch.int64)  # .to(x.device)
            search_pre_embed = self.motion_embedding(pre_seq_search) + self.motion_bias  # B, 4*pre_motion_num, C
        elif self.prompt_embed_type == 'sam':
            pre_seq_search = (search_pre_coor + 0.5).reshape(search_pre_coor.shape[0], search_pre_coor.shape[1], 2, 2)
            coords = (pre_seq_search + self.magic_num).to(torch.float)
            coords = 2 * coords - 1
            coords = coords @ self.positional_encoding_gaussian_matrix
            coords = 2 * np.pi * coords
            # outputs d_1 x ... x d_n x C shape
            search_pre_embed = torch.cat([torch.sin(coords), torch.cos(coords)], dim=-1)  # [batch,pre_motion_num,2,C]
            search_pre_embed = search_pre_embed.flatten(1, 2)  # B, 2*pre_motion_num, C
        else:
            raise ValueError


        if self.add_motion_pred:
            motion_pred_embedding = self.motion_pred_embedding.weight.unsqueeze(0).expand(B, -1, -1)
            x = torch.cat([x, search_pre_embed, motion_pred_embedding], dim=1)
        else:
            x = torch.cat([x, search_pre_embed], dim=1)
    ...
    def forward(self, z, x, search_pre_coor, concat_mode, inference_params=None):
        x = self.forward_features(z, x, search_pre_coor, concat_mode, inference_params=inference_params)
        # x = self.head(x)
        return x
...
if __name__ == '__main__':
    ...
    ckpt = torch.load('/home/lz/Downloads/pth/videomamba_m16_in1k_res224to448to576.pth')['model']
    del ckpt["head.weight"]
    del ckpt["head.bias"]
