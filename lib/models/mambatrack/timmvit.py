...
if __name__ == '__main__':
    ...
    # net = vit_tiny_patch16_224_in21k(img_size=h, class_token=False)
    net = vit_small_patch16_224_in21k(img_size=h, class_token=False)  # Number of trainable parameters: 23.162496M.  torch.Size([1, 4096, 384])
    # net = vit_base_patch16_224_in21k(img_size=h, class_token=False)
    ...
    # 统计显存使用量
    max_memory_allocated = torch.cuda.max_memory_allocated() / 1024**3  # 转换为 GB
    max_memory_cached = torch.cuda.max_memory_cached() / 1024**3  # 转换为 GB

    print(f"Max memory allocated: {max_memory_allocated:.2f} GB")
    print(f"Max memory cached: {max_memory_cached:.2f} GB")