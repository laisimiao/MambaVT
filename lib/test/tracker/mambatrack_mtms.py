...
from lib.models.mambatrack import build_mambatrack
...
class MambaTrack(BaseTracker):
    def __init__(self, params, dataset_name):
        super(MambaTrack, self).__init__(params)
        network = build_mambatrack(params.cfg, training=False)
    ...
    def track(self, image, info: dict = None):
        ...
        with torch.no_grad():
            ...
            # run the transformer
            out_dict = self.network.forward(template=template_list, search=[x_dict.tensors])
        ...
        if self.debug:
            ...
            cv2.putText(image_BGR, 'score:' + str(round(max_score, 3)), (40, 100),
