...
class ToGrayscale(TransformBase):
    ...
    def transform_image(self, image, do_grayscale):
        if do_grayscale:
            ...
            if image.shape[2] == 6:
                color_gray = cv.cvtColor(image[:, :, :3], cv.COLOR_RGB2GRAY)
                event_gray = cv.cvtColor(image[:, :, 3:], cv.COLOR_RGB2GRAY)
                return np.stack([color_gray, color_gray, color_gray, event_gray, event_gray, event_gray], axis=2)
            else:
