...
def crop_patch_from_image(im, target_bb):
    """ Extracts a crop at target_bb box, crop exactly what it is
...
def transform_image_to_crop(box_in: torch.Tensor, box_extract: torch.Tensor, resize_factor: float,
    ...
    returns:
        torch.Tensor - transformed co-ordinates of box_in xywh
    """
...
def transform_image_to_crop_batch(box_in: torch.Tensor, box_extract: torch.Tensor, resize_factor: float,
                            crop_sz: torch.Tensor, normalize=False) -> torch.Tensor:
    """ Transform the box co-ordinates from the original image co-ordinates to the co-ordinates of the cropped image
    args:
        box_in - the box for which the co-ordinates are to be transformed
    ...
    returns:
        torch.Tensor - transformed co-ordinates of box_in xyxy
    """
    box_extract_center = box_extract[0:2] + 0.5 * box_extract[2:4]
    ...
    box_out = torch.cat((box_out_center - 0.5 * box_out_wh, box_out_center + 0.5 * box_out_wh), dim=1)
    if normalize:
        return box_out / crop_sz[0]
    else:
        return box_out
...
def jittered_center_crop_premotion(frames, box_extract, box_gt, search_area_factor, output_sz, search_pre_coords, masks=None):
    """ For each frame in frames, extracts a square crop centered at box_extract, of area search_area_factor^2
    times box_extract area. The extracted crops are then resized to output_sz. Further, the co-ordinates of the box
    box_gt are transformed to the image crop co-ordinates
    ...
    args:
        ...
        search_area_factor - The area of the extracted crop is search_area_factor^2 times box_extract area
        output_sz - The size to which the extracted crops are resized
        search_pre_coords - list of boxes of same length as frames. [ (pre_motion_num, 4) * N_s ]
    ...
    box_crop = [transform_image_to_crop(a_gt, a_ex, rf, crop_sz, normalize=True)
                for a_gt, a_ex, rf in zip(box_gt, box_extract, resize_factors)]  # (x1,y1,w,h) list of tensors
    pre_coords = [transform_image_to_crop_batch(pre_gt, a_ex, rf, crop_sz, normalize=True)
                for pre_gt, a_ex, rf in zip(search_pre_coords, box_extract, resize_factors)]  # [ (pre_motion_num, 4) * N_s ]

    return frames_crop, box_crop, att_mask, masks_crop, pre_coords
