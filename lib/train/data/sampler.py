...
class TrackingSampler(torch.utils.data.Dataset):
    ...
    def __init__(self, datasets, p_datasets, samples_per_epoch, max_gap,
                 ...
                 train_cls=False, pos_prob=0.5, reverse_prob=0.4, pre_motion_num=0):
        """
        args:
        ...
        self.reverse_prob = reverse_prob  # reverse sequence for video-level training augmentation
        self.pre_motion_num = pre_motion_num  # number of pre motion coordinates frames
    ...
    def _sample_visible_ids(self, visible, num_ids=1, min_id=None, max_id=None,
                            allow_invisible=False, force_invisible=False, uniform_sample=False):
        ...
        if uniform_sample:
            idxes = np.linspace(0, len(valid_ids)-1, num=num_ids, dtype=int)
            return [valid_ids[i] for i in idxes]
    ...
    def getitem(self):
        ...
        while not valid:
            ...
            if is_video_dataset:
                ...
                if self.frame_sample_mode.startswith('causal'):
                    ...
                    while search_frame_ids is None:
                        ...
                        prev_frame_ids = self._sample_visible_ids(visible, num_ids=self.num_template_frames - 1,
                                                                  ...
                                                                  max_id=base_frame_id[0], uniform_sample='uniform' in self.frame_sample_mode)
                    ...
                    if reverse:
                        tmp_frame_ids = search_frame_ids[::-1] + template_frame_ids[::-1]
                        template_frame_ids = tmp_frame_ids[:self.num_template_frames]
                        search_frame_ids = tmp_frame_ids[-self.num_search_frames:]
            ...
            try:
                ...
                search_frames, search_anno, meta_obj_test = dataset.get_frames(seq_id, search_frame_ids, seq_info_dict, is_search=True, pre_motion_num=self.pre_motion_num, reverse=reverse)
                ...
                if "pre_coords" in search_anno:  # 代表的是第二阶段的训练
                    data = TensorDict({'template_images': template_frames,
                                    'template_anno': template_anno['bbox'],
                                    'template_masks': template_masks,
                                    'search_images': search_frames,
                                    'search_anno': search_anno['bbox'],
                                    'search_pre_coords': search_anno['pre_coords'],  # [ (pre_motion_num, 4) * N_s ] xyxy normalized
                                    'search_real_anno': search_anno['real_anno'],  # [ (4,) * N_s ] xywh raw image 
                    ...
                    data = TensorDict({'template_images': template_frames,
                                    ...
                                    'template_masks': template_masks,
                                    'search_images': search_frames,
                                    'search_anno': search_anno['bbox'],  # [ (4,) * N_s ]
                                    'search_masks': search_masks,
                                    'dataset': dataset.get_name(),
                                    'test_class': meta_obj_test.get('object_class_name')})
